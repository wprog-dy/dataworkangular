...
Class Driver_model extends CI_Model
{
	...
	{
		$query = $this->db->order_by('id', 'DESC')->get_where('drivers')->result();
		return $query;
    }
	function save_driver($save){
	   
		if ($save['id'] != ""){
			
			$this->db->where('id', $save['id']);
			$this->db->update('drivers', $save);

			$id	= $save['id'];
		}
		else{
			
			$this->db->insert('drivers', $save);
	...
	function get_driver($id){
		...
		$query = $this->db->get_where('drivers', array('id'=>$id))->row();
		return $query;
    }
